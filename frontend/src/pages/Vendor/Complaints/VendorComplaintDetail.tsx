...
import {
  X, Paperclip, Loader2, MessageSquare, CheckCircle2, ImagePlus, Trash2, Sparkles,
} from "lucide-react";
...
export function VendorComplaintDetail({ id, onClose }: Props) {
  ...
  const [aiAssist, setAiAssist] = useState<AiAssistResult | null>(null);
  ...
  async function handleAiAssist() {
    setAiLoading(true);
    try {
      const res = await api.get<{ data: AiAssistResult }>(`/api/vendor/complaints/${id}/ai-assist`);
      setAiAssist(res.data.data);
    } catch {
      // silently ignore
    } finally {
      setAiLoading(false);
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-5">
            ...
            <section>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground">AI Assist</p>
                {!aiAssist && (
                  <button
                    onClick={handleAiAssist}
                    disabled={aiLoading}
                    className="flex items-center gap-1.5 rounded-lg border border-violet-200 bg-violet-50 px-3 py-1 text-xs font-semibold text-violet-700 hover:bg-violet-100 transition-colors disabled:opacity-60"
                  >
                    {aiLoading
                      ? <><Loader2 className="h-3 w-3 animate-spin" /> Analyzing...</>
                      : <><Sparkles className="h-3 w-3" /> Translate &amp; Explain</>
                    }
                  </button>
                )}
              </div>
              ...
              {aiAssist ? (
                <div className="rounded-xl border border-violet-100 bg-violet-50/40 p-4 space-y-3">
                  {aiAssist.detected_lang !== "en" && (
                    <div>
                      <p className="text-[11px] font-semibold text-violet-700 mb-1 uppercase tracking-wide">
                        English Translation
                        <span className="ml-1.5 normal-case text-violet-500 font-normal">
                          (detected: {aiAssist.detected_lang === "ms" ? "Bahasa Malaysia" : aiAssist.detected_lang})
                        </span>
                      </p>
                      <p className="text-sm text-foreground leading-relaxed">{aiAssist.english_description}</p>
                    </div>
                  )}
                  <div>
                    <p className="text-[11px] font-semibold text-violet-700 mb-1 uppercase tracking-wide">Summary</p>
                    <p className="text-sm text-foreground leading-relaxed">{aiAssist.summary}</p>
                  </div>
                  {aiAssist.suggested_action && (
                    <div className="rounded-lg bg-violet-100/70 border border-violet-200 px-3 py-2.5">
                      <p className="text-[11px] font-semibold text-violet-800 mb-1 uppercase tracking-wide">Suggested Action</p>
                      <p className="text-sm font-medium text-violet-900 leading-relaxed">{aiAssist.suggested_action}</p>
                    </div>
                  )}
                  <button
                    onClick={() => setAiAssist(null)}
                    className="text-[11px] text-violet-400 hover:text-violet-600 underline underline-offset-2"
                  >
                    Clear
                  </button>
                </div>
                ...
                <div className="rounded-xl border border-dashed px-4 py-3 text-center">
                  <p className="text-xs text-muted-foreground">
                    Click "Translate &amp; Explain" to translate BM complaints and get AI-suggested actions
                  </p>
                </div>
              )}
            </section>
